...
function create_cluster() {
  ...
  then
    ...
    kind create cluster --name "test-haproxy-network-ingress-$BATS_TEST_ID" --wait "$CREATE_CLUSTER_COOLDOWN" &> /dev/null
  else
    BATS_CLEAN_TEST=false
  fi

  export KUBECONFIG=/tmp/test-haproxy-network-ingress-$BATS_TEST_ID

  # Delete cluster if BATS_CLEAN_TEST != false
  if [[ $BATS_CLEAN_TEST != "false" ]]
  then
    echo "kind delete cluster --name test-haproxy-network-ingress-$BATS_TEST_ID"
  fi
}
